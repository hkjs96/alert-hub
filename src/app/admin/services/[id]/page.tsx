    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="mb-2 text-sm font-semibold text-slate-700">
          서비스 담당 <span className="font-normal text-slate-400">(프로젝트 담당을 오버라이드)</span>
        </h2>
        <AssignmentEditor
          level="service"
          scopeId={service.id}
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="mb-2 text-sm font-semibold text-slate-700">AWS 계정 매핑</h2>
        ...
        <ul className="divide-y divide-slate-100 text-sm">
          {service.accounts.map((a: any) => (
            <li key={a.id} className="space-y-2 py-3">
              <div className="flex items-center gap-2">
                ...
                {a.alias ? <span className="text-slate-700">{a.alias}</span> : null}
                {a.environment ? (
          ...
          {service.accounts.length === 0 && (
            <li className="py-3 text-slate-400">매핑된 계정이 없습니다.</li>
          )}
        </ul>
      </section>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="mb-2 text-sm font-semibold text-slate-700">
          관련 인원 전체 <span className="font-normal text-slate-400">(계정 포함 롤업)</span>
        </h2>
