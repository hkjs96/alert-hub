...
- [ ] 홈닉 담당자에게 화면을 열어 줄 경우: 조직 트리 › 홈닉 패널 › **담당자 로그인 · 권한**에 `homenic.co.kr` 등록. 첫 로그인은 승인 대기에 "고객사 · 홈닉"으로 올라온다.
- [ ] 홈닉에 쓰기를 허용할지 결정(같은 화면의 체크박스: 담당자 관리 · 점검 창 등록 · Slack 채널 관리). 기본은 전부 읽기 전용.
...
- [ ] 고객사(홈닉)에 알람 내용을 Anthropic API 로 보내도 되는지 확인. 거부하면 고객사 화면 › "AI 메모 사용" 체크 해제.
- [ ] `ANTHROPIC_API_KEY` 를 Vercel 에 넣고 재배포 → 시스템 진단 › AI 메모 행이 "키 없음/꺼짐" 에서 "켜기" 가능 상태로.
- [ ] 시스템 진단 › AI 메모 › **켜기**. 해결 기록이 2건 이상 쌓인 서비스(또는 런북이 있는 서비스)의 다음 알람부터 스레드에 `🤖 AI 메모` 가 붙는다.
- [ ] 첫 주는 분류 적중(진단 행의 `분류 적중 x/y`)만 본다. 낮으면 끄고 기록을 더 쌓는다.

## 보류 (코드 쪽)

- [ ] 솔라피 문자·전화 사다리 — 발신번호 등록 뒤.
- [ ] AI 메모 2층(대화 요약: Resolve 때 Slack 스레드를 두 줄로) — Slack 앱에 `channels:history` 스코프가 필요해 별도. 3층(런북 절 초안 → 사람 승인)은 같은 해법이 반복되는 게 보이면.
