...
| `ANTHROPIC_API_KEY` | no | **AI 메모**의 Anthropic API 키. 키가 있어도 시스템 진단 › AI 메모 스위치(또는 `AI_INSIGHTS=on`)를 켜야 만든다. |
...
**기본값은 전부 읽기 전용**이고, MSP 관리자가 항목별로 켠다. 켜는 곳은 두 군데로 같은 폼이다:
**계정 · 접근 › 고객사 담당자 로그인 · 권한**(행을 펼치면 바로 편집)과 조직 트리 › 고객사 패널 › "담당자 로그인 · 권한".
...
- 고객사 밖의 기록은 근거로 쓰지 않는다. AI 메모(아래)는 이 표를 그대로 재료로 쓴다.

### AI 메모 (근거로만 쓴 한 단락)

통지가 나간 알람마다 `AlertInsight` 행이 pending 으로 놓이고, 통지 틱(`/api/cron/notify`)이 Claude Opus 5 에 한 번 물어
알람 상세의 **AI 메모** 카드와 Slack 스레드 한 줄(`🤖 AI 메모: … · 지난 3건 중 2건 재시작 · 먼저: …`)로 남긴다.
모델 응답이 수 초라 인라인으로 만들지 않고, 실패는 통지 잡과 같은 백오프(30s·1m·2m·5m·10m, 5회면 포기)로 재시도한다.
상세 화면의 **지금 생성 / 다시 생성** 버튼은 사람이 기다리는 중이니 인라인으로 만든다.

- **재료는 셋뿐**: 같은 고객사의 이전 처리 기록(E1…, 가까운 5건) · 런북 본문(R, 규칙 → 서비스) · 이 알람의 최근 이력(A).
  다른 고객사의 기록은 "같은 유형 N건"이라는 건수만 준다 — 내용은 격리.
- **근거 게이트**: 런북도 없고 기록도 2건 미만이면 모델을 부르지 않고 `skipped`("근거 부족")로 남긴다. 빈말도 비용도 없다.
- **출력은 믿지 않는다**: 구조화 JSON(`summary · likely_cause · next_steps[{text, evidence[]}] · suggested_kind · confidence`)을 받아,
  모르는 근거 id 를 단 다음 단계는 버리고, 제안 분류는 실제 기록이 2건 이상 그 분류일 때만 남긴다.
  화면의 신뢰도는 모델 점수가 아니라 건수다: **"지난 4건 중 3건 재시작"**.
- **채점은 결과로**: 사람이 분류를 기록하면(Slack 버튼·상세 라디오) 제안 분류와 같았는지가 `followed` 에 남는다.
  👍/👎 는 보조. 시스템 진단 › AI 메모 행에 `생성 12 · 근거 부족 4 · 👍 5 👎 1 · 분류 적중 3/4` 로 모인다.
- **스위치 두 겹**: 전역은 시스템 진단 › AI 메모(켜기/끄기, `AI_INSIGHTS` 환경변수보다 우선), 고객사별 거부는 고객사 화면의
  "AI 메모 사용" 체크(끄면 그 고객사 알람은 모델로 나가지 않는다 — 계약 확인 항목). 조회 전용·고객사 계정은 메모를 읽기만 한다.
- 요청 모양: `claude-opus-5`, `output_config.format` JSON 스키마, `effort: medium`, 시스템 프롬프트·런북은 캐시 블록,
  `fallbacks: "default"`(안전 분류기가 거부하면 서버가 대체 모델로 재시도). 토큰 사용량은 카드 접힘 줄에 보인다.

설계 배경과 다음 층(대화 요약 · 런북 승격 초안)은 [docs/aiops-market.md](docs/aiops-market.md) 5·7절.
