...
- 보는 것: 자기 고객사 알람만(대시보드·상세·이전 처리·런북)과 **우리 회사**(`/portal`, 읽기 전용: 프로젝트 › 서비스 › AWS 계정과 담당 순서, 담당자 명단, Slack 채널, 런북, 진행 중 점검 창).
  Ack/Resolve/뮤트 버튼은 비활성, 등록 관리 탭 대신 "우리 회사" 탭, Slack 버튼도 거부. 관리자는 `/portal?customer=<id>` 로 같은 화면을 미리 본다.
