...
| `AUTH_AUTO_APPROVE` (진단 화면의 "가입 방식" 스위치가 우선) | no | `true`면 허용 목록 계정은 승인 없이 로그인 즉시 온콜 엔지니어로 활성. 기본(승인제)은 관리자가 팀 · 내부 인원에서 승인한다. |
...
- **가입 방식 스위치**: 등록 관리 › 인증 › "가입 방식"에서 **승인제 / 자동 승인**을 고른다. 화면 값이 `AUTH_AUTO_APPROVE` 환경변수보다 우선하고 DB(`Setting`)에 남는다.
- **승인 대기 목록**: 같은 화면 아래(항상 보임)와 팀 · 내부 인원 맨 위(대기가 있을 때). 내부 인원은 역할을 고르고 승인, 고객사 담당자는 "고객사 · 이름" 배지와 함께 조회 전용으로 승인. 거절도 여기서.
...
발송 파이프가 없는 채널은 버튼이 "서버 미설정"으로 잠기고, 진단 화면(등록 관리 › 인증)에 Slack 봇·SMTP·SMS 공급자 행이 보인다.
...
6. 로그인 오류는 사용자 언어와 참조 코드(`AU-xxxxx`)만 화면에 나가고, 원인은 서버 로그에 같은 코드로 남는다.
   환경변수 상태·허용 목록·미설정 경고는 관리자 전용 **등록 관리 › 인증** 진단 화면에만 나타난다.
